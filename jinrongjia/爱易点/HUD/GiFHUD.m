...
@property (nonatomic, strong) UIView *fullOverlayView;
...
- (instancetype)init {
    if ((self = [super initWithFrame:CGRectMake(0, 0, Size, Size)])) {
        ...
        //[self.imageView.layer setCornerRadius:30];
...
+ (void)showWithOverlay {
    //[self dismiss:^{
        [APPDELEGATE.window addSubview:[[self instance] getOverlay]];
        [self show];
    //}];
}

+ (void)showWithFullOverlay {
    //[self dismiss:^{
        [APPDELEGATE.window addSubview:[[self instance] getFullOverlay]];
        [self show];
    //}];
}
...
+ (void)dismiss {
    ...
    [[[self instance] getOverlay] removeFromSuperview];
    [[[self instance] getFullOverlay] removeFromSuperview];
...
- (UIView *)overlay {
    return [self getOverlay];
}

- (UIView *)getFullOverlay {
    if (!self.fullOverlayView) {
        self.fullOverlayView = [[UIView alloc] initWithFrame:APPDELEGATE.window.frame];
        [self.fullOverlayView setBackgroundColor:[UIColor colorWithRed:94.0/255.0 green:94.0/255.0 blue:94.0/255.0 alpha:1]];
        [self.fullOverlayView setAlpha:0];
        
        [UIView animateWithDuration:FadeDuration animations:^{
            [self.fullOverlayView setAlpha:1];
        }];
    }
    return self.fullOverlayView;
}
...
- (UIView *)getOverlay {
    if (!self.overlayView) {
        ...
        //[self.overlayView setBackgroundColor:[UIColor colorWithRed:94.0/255.0 green:94.0/255.0 blue:94.0/255.0 alpha:1]];
