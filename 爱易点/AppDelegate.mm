...
@property (strong, atomic) CBCentralManager *cbmanager;
...
- (BOOL)initLocationService {
    if([CLLocationManager locationServicesEnabled] && [CLLocationManager authorizationStatus] == kCLAuthorizationStatusNotDetermined) {
        ...
        SEL requestSelector;
        if (ALWAYS_AUTH)
            requestSelector = NSSelectorFromString(@"requestAlwaysAuthorization");
        else
            requestSelector = NSSelectorFromString(@"requestWhenInUseAuthorization");
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
    switch (central.state) {
        ...
        case CBCentralManagerStateUnsupported:
            BTON = NO;
            //[HTTPRequest alert:@"对不起，UniCafe需要蓝牙4.0，所以iPhone4S或之后的型号才能运行"];
            break;
            
        case CBCentralManagerStateUnauthorized:
            BTON = NO;
            //[HTTPRequest alert:@"UniCafe需要使用蓝牙，请在系统设置中授权UniCafe"];
            break;
            
        case CBCentralManagerStatePoweredOff:
            BTON = NO;
            //[HTTPRequest alert:@"UniCafe需要使用蓝牙，请在系统设置中打开蓝牙"];
            break;
             */
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if (![self initLocationService]) {
        locationOn = NO;
        //[HTTPRequest alert:@"UniCafe需要位置服务以判断您所在的餐厅。\n请在系统设置->隐私->位置服务中打开。"];
    } else {
        locationOn = YES;
    }
    
    //bluetooth, comment: if show notification at launch
    self.cbmanager = [[CBCentralManager alloc] initWithDelegate:self queue:nil/* options:@{CBCentralManagerOptionShowPowerAlertKey:[NSNumber numberWithBool:NO]}*/];
    
    /*
    self.myBeaconManager = [[ESTBeaconManager alloc] init];
    //iOS 8
    [self.myBeaconManager requestAlwaysAuthorization];
     */
    /*
    _mapManager = [[BMKMapManager alloc] init];
    BOOL ret = [_mapManager start:BAIDU_MAP_KEY generalDelegate:nil];
...
- (void)onLocateBeacons:(ESTBeaconManager *)manager didRangeBeacons:(NSArray *)beacons inRegion:(ESTBeaconRegion *)region {
    ...
    NSDate *lastDate = [[NSUserDefaults standardUserDefaults] objectForKey:@"background_date"];
    if (lastFound == nil)
        lastFound = @"";
    if (lastDate == nil) {
    ...
    if (![notificationInfo isEqualToString:lastFound] || ABS([now timeIntervalSinceDate:lastDate]) > 5 * 60 * 60) { //5 hours
        ...
        [[NSUserDefaults standardUserDefaults] setObject:now forKey:@"background_date"];
        [[NSUserDefaults standardUserDefaults] synchronize];
        
        //show notification
        [self popNotification:notificationInfo];
    }
}
...
- (BOOL)myHandleURL:(NSURL*)url {
    ...
    if ([url absoluteString].length >= 7 && [[[url absoluteString] substringToIndex:7] isEqualToString:@"unicafe"]) {
        return YES;
    } else if ([url absoluteString].length >= 10 && [[[url absoluteString] substringToIndex:10] isEqualToString:@"unipingpay"]) {
...
- (void)applicationWillEnterForeground:(UIApplication *)application {
    ...
    if (ABS(timeGap) > 60 * 60) { //1 hour
        ...
        [store setCurrentStore:[store getCurrentStoreID]];
        [storeList refreshMenuAndOrder];
        lastRefreshTime = [NSDate date];
    }
}
