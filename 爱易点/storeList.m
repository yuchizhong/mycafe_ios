...
- (void)goStart {
    ...
    //if (DEBUG_MODE) {
        //simulate a beacon after 5 seconds if not found one
...
- (void)test {
    ...
    if (i < 3)
        ...
        [self processBeacon:MAJOR3 andMinor:MINOR3];
    i++;
    if (i > 9)
        i = 0;
     */
...
- (void)loadPageAsync {
    ...
    [NSThread detachNewThreadSelector:@selector(checkCanOrder) toTarget:self withObject:nil];
...
- (void)checkCanOrder {
    if (self.usingThreadsCheck > 0) {
        return;
    }
    self.usingThreadsCheck++;
    while (true) {
        //预定时不判断
        if (![store preorder_mode]) {
            if (myOrder != nil &&
                (([((UIBarButtonItem*)[myOrder.navigationItem.rightBarButtonItems objectAtIndex:0]).title isEqualToString:@"下单"] && ![store inStore]) ||
                 ([((UIBarButtonItem*)[myOrder.navigationItem.rightBarButtonItems objectAtIndex:0]).title isEqualToString:@"店外"] && [store inStore]))) {
                    [self performSelectorOnMainThread:@selector(refreshMenuAndOrderLocal) withObject:nil waitUntilDone:YES];
                }
            if (![store inStore]) {
                [self performSelectorOnMainThread:@selector(getOutStore) withObject:nil waitUntilDone:YES];
                if (self.inStore == YES && [[UIApplication sharedApplication] applicationState] == UIApplicationStateActive) {
                    //alert
                    [self performSelectorOnMainThread:@selector(alertOutOfStore) withObject:nil waitUntilDone:YES];
                }
                self.inStore = NO;
            } else {
                [self performSelectorOnMainThread:@selector(getInStore) withObject:nil waitUntilDone:YES];
                self.inStore = YES;
            }
        } else {
            self.inStore = NO;
